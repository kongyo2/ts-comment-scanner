...
- **Position-dependent directives are never activated by a removal**: when deleting a leading comment would make prettier's `@format`/`@prettier` pragma (honoured only in the file's first comment) or Bun's `// @bun` marker (honoured only at the very start of the file) newly take effect in a kept comment, the leading comment is kept as well; likewise, when deleting a block comment sitting between a kept comment and a declaration would let knip's `@public` and similar tags (attached only when a token follows the comment directly) newly reach that declaration, the block comment is kept
...
- **Compiler / types**: `@ts-ignore` `@ts-expect-error` `@ts-nocheck` `@ts-check` / `/// <reference>` / `@internal` (`stripInternal`) / `@ts-strict-ignore`, `@ts-strict` (typescript-strict-plugin) / `@deno-types=`, `@ts-types=`, `@ts-self-types=` (Deno)
- **Linters**: the `eslint-disable` family, `eslint-env`, `/* global */`, `/* exported */` / the `oxlint-disable` family / the `stylelint-disable` family, `stylelint-enable` (stylelint) / the `biome-ignore` family, `rome-ignore` / the `deno-lint-ignore` family / `@flow`, `@noflow`, the `$FlowFixMe` family, `flowlint` (Flow) / `tslint:`, `jshint`, `jscs:`, `jslint` (legacy)
- **Unused-code detection**: `@public`, `@beta`, `@alias`, `@internal`, `@lintignore` (knip's JSDoc/TSDoc tags; block comments only, honoured when a token follows the comment directly)
